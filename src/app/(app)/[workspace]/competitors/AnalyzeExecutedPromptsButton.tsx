...
export function AnalyzeExecutedPromptsButton({ workspaceId }: Props) {
  ...
  function handleAnalyze() {
    startTransition(async () => {
      ...
      const { analyzedRuns, detectedCandidates, createdCompetitors, createdSuggestions } =
        result.data;
    ...
    <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5 space-y-3">
      <div>
        <h2 className="text-sm font-semibold text-slate-700">
          Extraer competidores de prompts ejecutados
        </h2>
        <p className="text-xs text-slate-500 mt-1">
          Analiza respuestas históricas, crea competidores detectados varias veces y deja el resto
          como sugerencias.
        </p>
      </div>
