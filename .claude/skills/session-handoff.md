...
1. Plan files referenced this session (check `C:\Users\David\.claude\plans\` if a plan was mentioned).
2. TodoWrite state — any in-progress or pending tasks.
3. Background processes you started with `run_in_background` — shell IDs are load-bearing for the next agent.
4. Files created or modified this session — you know what you touched; don't grep to re-discover.
5. Memory files written or updated (`C:\Users\David\.claude\projects\C--Users-David-VS-neo-geo-2\memory\`).
...
**Do NOT audit the filesystem.** This is synthesis of what happened in THIS session. No git log, no broad Glob sweeps. If you didn't touch it this session, it doesn't belong here.
