-- 0028: Añade p_country_filter a las 5 RPCs del dashboard para que el filtro
-- de país se propague a todos los paneles de análisis.
-- Patrón: JOIN prompts p ON pr.prompt_id = p.id +
...
grant execute on function get_workspace_market_share(text, int, text, text) to authenticated, service_role;
...
grant execute on function get_workspace_mention_breakdown(text, int, text, text) to authenticated, service_role;
...
grant execute on function get_workspace_top_competitors(text, int, int, text, text) to authenticated, service_role;
...
grant execute on function get_workspace_top_sources(text, int, int, text, text) to authenticated, service_role;
...
grant execute on function get_workspace_llm_comparison(text, int, text) to authenticated, service_role;
